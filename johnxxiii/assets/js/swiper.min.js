...
//---------------------------swipers-start----------------------------------------
let header_slider = document.querySelectorAll('.header-full-menu__titles-swiper');
if (header_slider.length) {
    let headerSwiper = new Swiper('.header-full-menu__titles-swiper', {
        speed: 300, spaceBetween: 0, slidesPerView: 'auto', freeMode: true,
...
//-------------------------------------------
let simple_labels_slider = document.querySelectorAll('.swiper-simple-labels-js');
if (simple_labels_slider.length) {
    simple_labels_slider.forEach((item) => {
        let simple_labels_slider = new Swiper(item, {
            speed: 300, spaceBetween: 0, slidesPerView: 'auto', freeMode: true, threshold: 15,
        });
        window.addEventListener('load', () => {
            simple_labels_slider.update();
        });
    });
}
//-------------------------------------------
let hero_bottom_swiper_nav = document.querySelector('.swiper-second-labels-js.s-hero__bottom-nav');
if (window.innerWidth < 1200 && hero_bottom_swiper_nav) {
    hero_bottom_swiper_nav.classList.remove('swiper-second-labels-js');
}
let second_labels_slider = document.querySelectorAll('.swiper-second-labels-js');
if (second_labels_slider.length) {
    second_labels_slider.forEach((item) => {
        let second_labels_slider = new Swiper(item, {
            speed: 300, spaceBetween: 0, slidesPerView: 'auto', freeMode: true,
            threshold: 15,
            navigation: {
        ...
        window.addEventListener('load', () => {
            second_labels_slider.update();
        });
    });

}
//-------------------------------------------
const sliders_blocks = document.querySelectorAll(".complex-sliders-block-js");
if (sliders_blocks.length) {
    sliders_blocks.forEach((item) => {
        const pics = item.querySelector(".slider-pics-js");
        const contents = item.querySelector(".slider-contents-js");
        // function slider_height() {
        //     let content_height = contents.offsetHeight,
        //         pic_height = pics.offsetHeight;
        //     if(content_height > pic_height) {
        //         pics.style.height = content_height + 'px';
        //     }
        // }
        ...
        //     const themeClass = activeSlide.getAttribute("data-class-theme");
        //     if (themeClass) {
...
//-------------------------------------------
const sliders_blocks_2 = document.querySelectorAll(".complex-sliders-block-2-js");
if (sliders_blocks_2.length) {
    sliders_blocks_2.forEach((item) => {
        const pics = item.querySelector(".slider-pics-2-js");
        const contents = item.querySelector(".slider-contents-2-js");
        const picsSwiper = new Swiper(pics, {
            effect: "fade",
            fadeEffect: { crossFade: true },
            parallax: true,
...
//-------------------------------------------
const sliders_blocks_3 = document.querySelectorAll(".complex-sliders-block-3-js");
if (sliders_blocks_3.length) {
    sliders_blocks_3.forEach((item) => {
        const pics = item.querySelector(".slider-pics-3-js"),
            contents = item.querySelector(".slider-contents-3-js"),
            thumbs = item.querySelector(".slider-thumbs-3-js");
        if (window.innerWidth > 991) {
            const picsSwiper = new Swiper(pics, {
                // direction: 'vertical',
                effect: "fade",
            ...
            // Update when slide changes (prev/next/pagination)
            contentSwiper.on('slideChange', function () {
                updateActiveThumb(contentSwiper.realIndex);
            });
            ...
            let slides = thumbs.querySelectorAll('.s-pre-k__blocks-swiper-slide');
            slides.forEach((titleEl) => {
                titleEl.addEventListener('mouseenter', () => {
                    slides.forEach(el => el.classList.remove('is-active'));
                    titleEl.classList.add('is-active');
                    const index = titleEl.dataset.id;
...
//-------------------------------------------
const overview_sliders = document.querySelectorAll(".overview-sliders-js");
if (overview_sliders.length) {
    overview_sliders.forEach((item) => {
        const pics = item.querySelector(".overview-slider-1-js");
        const contents = item.querySelector(".overview-slider-2-js");
        const contentSwiper = new Swiper(contents, {
            effect: "fade",
            fadeEffect: { crossFade: true },
            speed: 900,
        ...
        const picsSwiper = new Swiper(pics, {
            ...
            pagination: {
                el: item.querySelector(".swiper-pagination--fraction"),
                type: "fraction",
...
//-------------------------------------------
document.querySelectorAll('.menu__content-with-slider-js').forEach((container) => {
    const titles = container.querySelectorAll('.menu__child-list-item-arrow-js .menu__child-list-link-js');
    const swiperEl = container.querySelector('.menu__sub-child-swiper-js');

    if (!swiperEl) return;

    const contentSwiper = new Swiper(swiperEl, {
        effect: "fade",
        fadeEffect: { crossFade: true },
        speed: 900,
        allowTouchMove: false,
    ...
    titles.forEach((titleEl) => {
        titleEl.addEventListener('mouseenter', () => {
            const targetSelector = titleEl.dataset.id;
            if (!targetSelector) return;

            const targetEl = container.querySelector(targetSelector);
            if (!targetEl) return;

            const slideEls = Array.from(swiperEl.querySelectorAll('.swiper-slide'));
            const index = slideEls.indexOf(targetEl);
            if (index >= 0) {
                contentSwiper.slideTo(index);
            }
        });
...
//-------------------------------------------
let cards_slider = document.querySelectorAll('.cards-swiper-js');
if (cards_slider.length) {
    cards_slider.forEach((item) => {
        let valuesSwiper = new Swiper(item, {
            speed: 900,
            slidesPerView: 'auto', spaceBetween: 1,
        });
    });
}
//-------------------------------------------
let news_block = document.querySelectorAll('.news-block-js');
if (news_block.length) {
    news_block.forEach((item) => {
        let cat_news = item.querySelector('.news-block__cat-news-js'),
            cat_events = item.querySelector('.news-block__cat-events-js'),
            btn_news = item.querySelector('.news-block__btn-news-js'),
            btn_events = item.querySelector('.news-block__btn-events-js'),
            slider_cats = item.querySelector('.news-block__cats-slider-js'),
            slider_news = item.querySelector('.news-block__news-slider-js'),
            slider_events = item.querySelector('.news-block__events-slider-js'),
            news_pag = item.querySelector('.news-block__news-btns-js'),
            events_pag = item.querySelector('.news-block__events-btns-js');

        const catsSwiper = new Swiper(slider_cats, {
            effect: "fade",
            fadeEffect: { crossFade: true },
            speed: 900,
            allowTouchMove: false,
        });
        let is_parallax_imgs = !!item.querySelectorAll('.swiper-slide .slide-image').length > 0;
        let newsSwiper = new Swiper(slider_news, {
            speed: 900, threshold: 15, slidesPerView: 'auto', spaceBetween: 1,
            navigation: {
                nextEl: news_pag.querySelector('.swiper-button-next'),
                prevEl: news_pag.querySelector('.swiper-button-prev'),
            },
        });
        let eventsSwiper = new Swiper(slider_events, {
            speed: 900, threshold: 15, slidesPerView: 'auto', spaceBetween: 1,
            navigation: {
                nextEl: events_pag.querySelector('.swiper-button-next'),
                prevEl: events_pag.querySelector('.swiper-button-prev'),
            },
        });
        cat_events.addEventListener('click', (e) => {
            switchCats(1);
        });
        cat_news.addEventListener('click', (e) => {
            switchCats(0);
        });
        function switchCats(index) {
            cat_events.classList.toggle('is-active', index === 1);
            cat_news.classList.toggle('is-active', index === 0);
            btn_events.classList.toggle('is-active', index === 1);
            btn_news.classList.toggle('is-active', index === 0);
            events_pag.classList.toggle('is-active', index === 1);
            news_pag.classList.toggle('is-active', index === 0);
            catsSwiper.slideTo(index);
        }
    });
}
//-------------------------------------------
const simple_sliders = document.querySelectorAll('.simple-slider-js');
let lastTime = performance.now();
let lastTranslate = 0;
let velocity = 0;
let rafId = null;
if (simple_sliders.length) {
    simple_sliders.forEach((item) => {
        let is_parallax_imgs = !!item.querySelectorAll('.swiper-slide .slide-image').length > 0;
        let spaceBetween = 30;
        if (item.classList.contains('simple-slider--secondary-js') && window.innerWidth > 991) {
            spaceBetween = 40;
        }
        const simpleSwiper = new Swiper(item, {
            speed: 900, spaceBetween: spaceBetween,
            // allowTouchMove: false,
            resistance: false, slidesPerView: 'auto', navigation: {
...
function trackVelocity(swiper) {
    ...
    // console.log('Velocity:', velocity.toFixed(3), 'px/ms');
...
function simulateDragTracking(swiper) {
    ...
    const fakeTrack = () => {
        ...
        if (velocity > 0.01) {
            ...
            console.log('⛔️ Simulated motion stopped');
            stopTracking();
        }
    };
    rafId = requestAnimationFrame(fakeTrack);
...
function applyScaleEffect(velocity, swiper) {
    ...
    swiper.el.querySelectorAll('.swiper-slide .slide-image').forEach((img) => {

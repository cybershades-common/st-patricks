...
document.querySelectorAll('a[href*="youtube.com/shorts/"]').forEach(link => {
    let shortUrl = link.href;
    let videoId = shortUrl.split("/shorts/")[1].split("?")[0];
    if (videoId) {
...
document.querySelectorAll('a[href*="youtube.com"], a[href*="youtu.be"]').forEach(function (link) {
    ...
    if (url.hostname.includes("youtu.be") || (url.hostname.includes("youtube.com") && url.searchParams.has("v"))) {
        link.classList.add("venobox-link");
        link.setAttribute("data-autoplay", "true");
        link.setAttribute("data-vbtype", "video");
    }
});
let venobox_link = document.querySelectorAll('.venobox-link');
if(venobox_link.length) {
    new VenoBox({
        ...
        onContentLoaded: function(newcontent){
            ...
            // const ratioBox = document.querySelector('.vbox-child');
            // if (closeBtn && ratioBox) {
            //     const clonedBtn = closeBtn.cloneNode(true);
            //     ratioBox.appendChild(clonedBtn);
            // }
        },
    });
}

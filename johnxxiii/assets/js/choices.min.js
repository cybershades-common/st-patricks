...
if (selectElements.length && choices.length === 0) {
    selectElements.forEach(select => {
        ...
        const choice = new Choices(select, {
            removeItemButton: use_removeItemButton,
            renderSelectedChoices: false,
            searchEnabled: true,
            itemSelectText: '',
            shouldSort: false,
            searchPlaceholderValue: 'Enter a value to search',
            callbackOnInit: function() {
                // console.log(this.dropdown.element);
                this.dropdown.element.setAttribute('data-lenis-prevent', '');
                this.dropdown.element.setAttribute('data-scroll', 'exclude');
            }
        });
        select.addEventListener('change', () => {
            ...
            wrapper.classList.toggle('is-selected', hasValue);
        });
        const wrapper = select.closest('.choices');
        if (wrapper && select.value && select.value.trim() !== '') {

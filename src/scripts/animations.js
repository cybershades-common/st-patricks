...
function initAboutSeparatorAnimation() {
  ...
  const separators = document.querySelectorAll('.about-glenaeon-section__separator');
  if (!separators.length) return;

  const smoother = typeof ScrollSmoother !== 'undefined' ? ScrollSmoother.get() : null;
  const scrollerEl = smoother ? smoother.wrapper() : null;

  // Animate each separator individually with its own ScrollTrigger
  separators.forEach((separator, index) => {
    const separatorSvg = separator.querySelector('svg');
    const revealRect = separator.querySelector('.about-glenaeon-section__separator-reveal');
    if (!separatorSvg || !revealRect) return;

    // Make sure each separator has a unique clipPath ID
    const clipPath = separatorSvg.querySelector('clipPath');
    const clipTarget = separatorSvg.querySelector('[clip-path]');
    if (clipPath && clipTarget) {
      const clipId = `about-separator-clip-${index}`;
      clipPath.id = clipId;
      clipTarget.setAttribute('clip-path', `url(#${clipId})`);
    }
...
`);
  ...
  animateHero(el, config) {
    ...
    const script = slide.querySelector('.hero__title-script');
    const main = slide.querySelector('.hero__title-main');
    const descriptions = slide.querySelectorAll('.hero__description'); // Get ALL descriptions (mobile + desktop)
    const playButton = el.querySelector('.play-button');
    ...
    if (script) script.style.cssText = 'display:block;overflow:visible;opacity:0';
    if (main) main.style.cssText = 'display:block;overflow:visible;opacity:0';
    descriptions.forEach(desc => { if (desc) desc.style.opacity = '0'; });
    ...
    const setup = () => {
      ...
      descriptions.forEach(desc => {
        if (desc) gsap.set(desc, { y: DESCRIPTION.fromY });
      });
      if (playButton) {
...
function initFooterParticles() {
  ...
  function getMousePositionInFooter(e) {
  ...
  footer.addEventListener('mousemove', function (e) {
    if (!isMouseOverFooter) return;

    const pos = getMousePositionInFooter(e);
  ...
  function animate() {
    // Fade out existing particles for trail effect while keeping canvas transparent
...
// Initialize footer particles when DOM is ready

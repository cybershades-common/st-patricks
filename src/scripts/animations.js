...
class GSAPAnimations {
      ...
      switch (animation) {
        ...
        case 'image-clip-right':
          elements.forEach((el, i) => {
            const elCfg = { ...cfg, delay: (parseFloat(cfg.delay) || 0) + i * (parseFloat(cfg.stagger) || 0), stagger: null };
            this.animateSingle(el, animation, elCfg);
          });
          break;
        default:
  ...
  // Card row reveal: clip from top (v2 of card-row-stagger)
  cardRowClipTop(el, cfg) {
    if (!el) return;
    ...
    if (container.hasAttribute('data-card-row-clip-top-initialized')) return;
    container.setAttribute('data-card-row-clip-top-initialized', 'true');
    ...
    sortedRows.forEach((row) => {
      row.forEach((card, cardIndexInRow) => {
        ...
        // Divider — same as cardRowStagger
        if (divider) {
